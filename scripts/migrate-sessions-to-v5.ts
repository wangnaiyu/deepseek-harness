...
Options:
  ...
  --legacy-pto-v4     Root contains pre-upgrade PTO V4 (must not mix upstream V4)
  --help              Show this help
`
...
async function migrate(root: string, jobs: number, legacyPtoV4: boolean): Promise<number> {
...
if (process.argv[1] !== undefined && realpathSync(process.argv[1]) === realpathSync(import.meta.filename)) {
  try {
    const { values } = parseArgs({ options: { 'sessions-dir': { type: 'string' }, jobs: { type: 'string' }, help: { type: 'boolean' }, 'legacy-pto-v4': { type: 'boolean' } }, strict: true })
    if (values.help) console.log(usage)
      ...
      process.exitCode = await migrate(resolve(values['sessions-dir'] ?? join(homedir(), '.dsh', 'sessions')), jobs, values['legacy-pto-v4'] === true)
    }
  } catch (error: unknown) {
