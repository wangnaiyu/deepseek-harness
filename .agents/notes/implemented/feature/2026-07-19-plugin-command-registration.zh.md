...
`CommandDefinition` 包含不带 `/` 的小写名称、非空描述、可选的非结构化输入提示、供可信 Host ownership projection（归属投影）使用的可选不透明 provider identity（提供方身份），以及可取消处理器。provider identity 不影响优先级或执行，也不会进入普通 UI 描述符。注册会校验元数据并复制一份与调用方脱离的副本、冻结最终生效的定义，并返回对应 Cordis effect 的精确 disposer（资源释放器）。同一层中的重复名称会失败。每个消费方都能看到所有有效定义；若命令插件无法在某种部署中运行，它就不在该部署中注册，而不是把消费方身份编码进共享领域。

`list(agent)` 在作用域遮蔽后返回不可变、按名称排序的描述符。没有 Agent 的 Host 发现消费方使用 `listGlobalDescriptors()` 读取全局层，使用 `listForScope(scope)` 读取有效的 standing-scope（常驻作用域）视图，或在需要有效 winner（胜出项）的注册 layer 与 provider identity 时使用 `listDiscoveryForScope(scope)`。发现投影在合并期间记录 provenance（来源事实），而不是比较公开描述符，因为 scoped override（作用域覆盖）与全局 fallback（回退项）可以完全相同。所有读取都只读且不需要 Session。`find(agent, name)` 解析有效定义。`execute(agent, line, signal)` 解析并运行已知定义，返回分离后的 `success` 或 `error` 结果；无效语法和未知名称返回 `undefined`，由适配器负责生成直接展示的错误文本。
...
- **创建临时 Agent 来发现作用域命令**——不予采纳，因为发现可以直接寻址 standing scope；为只读目录构造 Agent 会跨越 Session 生命周期边界。

## 后果

- 命令生产者是普通的可移除插件，TUI 消费其经过校验的目录与分派约定。
- 特定于 agent 的定义保留现有扁平作用域与遮蔽语义，不引入核心到 UI 的依赖。
- Host 发现可以解析全局与 standing-scope 命令 winner 及其注册 provenance，而不激活 Agent 或 Session。
