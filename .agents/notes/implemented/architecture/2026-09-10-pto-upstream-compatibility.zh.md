...
0.1.6 适配保留上游主面板外壳、权限选择器、草稿编辑器运行时和最近更新／手动排序引擎。PTO 草稿状态接入它们的当前契约。隐藏的空 Session 在首条消息前保留手动排序身份。SSH 目录预留传递逐次调用策略，并保留已有目标拒绝语义。已有 PTO receipt 历史读取按上游弃用政策记录为待迁移投影。
