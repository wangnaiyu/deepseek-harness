...
- **ui-skill**：`skills/list({sessionId})` 按会话寻址（host 从会话 header 解析项目根）；目录缓存按 sessionId 键控 single-flight，`warm` 钩子出生预热、`connection/reset` 全清。pick 产出规范 `/skill <name> ` 语法的 text outcome；`lexicon` 从 CatalogFetch 的 settled 快照同时提供规范和旧兼容词形（未热 `undefined`），`subscribeLexicon` 在 settle 与失效时按会话通知监听者。无 match 钩子（引用不进命令裁决）。skill 引用以原文随普通提示词走；`dsh-tool-skill` 在 Host pre-step 解析规范手势，旧 `/<name>` 仅在有效命令不存在同名项时接受。
