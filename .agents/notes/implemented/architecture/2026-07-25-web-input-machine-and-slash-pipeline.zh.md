...
- `decorations.scanTextRefs`：词边界、最长词形优先地扫描 draft（行首/空白后的 `/skill <name>`、旧 `/name` 和 `@name`，`x/name` 永不命中）对照名录，命中即 `.textRef` mark（backdrop 纯 range 高亮，同 hlToken）；编辑破坏匹配形状下次扫描自然消失。
