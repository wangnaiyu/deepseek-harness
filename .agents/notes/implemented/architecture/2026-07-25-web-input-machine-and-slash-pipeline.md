...
- `decorations.scanTextRefs`: a word-boundary, longest-lexeme scan of the draft (`/skill <name>`, legacy `/name`, and `@name` at line start / after whitespace; `x/name` never hits) against the roster; a hit gets the `.textRef` mark (a pure range highlight on the backdrop, same as hlToken); an edit breaking the match shape simply disappears on the next scan.
