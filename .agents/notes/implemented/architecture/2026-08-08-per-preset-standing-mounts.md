...
- **Host readers address both registries and isolated services through the standing mount.** `standingKeyFor()` supplies the scope-chain key, while `serviceForStanding()` resolves a service whose provider lives inside the mount's entry-local realm. The latter returns absence instead of substituting a same-named host-root service, so a no-Agent read preserves the preset's actual composition semantics.
