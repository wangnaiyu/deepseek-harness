...
本记录保存上游 PR #5073 合并树（0.1.7-rc.1）的完整 V4 类型。它不描述旧 PTO fork 的同号 V4；旧 fork 声明独立保存在 legacy-pto-v4 目录。
...
直接从上游 PR #5073 合并树提取 docs/persistence-schema.json，保留摘要与类型，去除源码行号。本记录不是发布声明。

<a id="characteristics"></a>
## 格式特征

上游 V4 将工具结果转为 tool-role 消息，使用直接来源标识，并定义 developer 消息和父会话目录。物理编码与关系规则见 [V3-to-V4](../../../packages/session/session-format-v3-to-v4/README.zh.md).
...
本地 verify-persistence-formats 验证完整性、声明与生成内容；类型快照不等于历史应用回放或迁移安全证据。旧 PTO V4 使用 V3 事件体，必须显式选择来源。
