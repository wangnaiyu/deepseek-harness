---
description: "PTO V3 集成 schema 与来源检查点。"
...
查看 PTO 集成检查点在 V4 writer 迁移之前的 V3 schema，其中同时包含 fork 来源变体、上游 system-message 与规范事件 envelope 声明。
...
来源 tag `dsh-pto-v3-integration-checkpoint-20260915` 标识经过验证的 V3 源码。它是集成检查点，不是产品发布；归档在修改 writer 常量前提取。

<a id="characteristics"></a>
## 格式特征

V3 使用逻辑 `isSeeded`、规范 surface 替换端点与受保护的 system-message 头。[V3 codec](../../../packages/session/session-format-v2-to-v3/README.zh.md) 拥有物理准入和继承边界；本 fork 检查点包含 PTO 来源元数据。
...
[完整机器目录](v3.schema.json)包含 60 个根类型和 472 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。
...
来源：`packages/host/pto-artifact-inspection/src/types.ts`
...
| `index` | 必需 | `number` |
| `texts` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.artifactRefs`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptartifactrefs) |
| `time0` | 必需 | `number` |
| `type` | 必需 | `"text-chunks"` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `args` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.artifactRefs`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptartifactrefs) |
| `dt` | 必需 | [`SessionEventEnvelope[39].sourceEventSeqs`](#persistence-type-sessioneventenvelope39sourceeventseqs) |
| `id` | 必需 | `string` |
| `index` | 必需 | `number` |
| `name` | 可选 | `string` |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `allow` | 可选 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.artifactRefs`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptartifactrefs) |
| `deny` | 可选 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.artifactRefs`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptartifactrefs) |
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `blockedBy` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.artifactRefs`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptartifactrefs) |
| `description` | 必需 | `string` |
| `id` | 必需 | `string` |
| `ownerId` | 可选 | `string` |
| `revision` | 必需 | `number` |
...
| `subject` | 必需 | `string` |
| `writeScopes` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.artifactRefs`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptartifactrefs) |
...
来源：`packages/host/pto-artifact-inspection/src/index.ts`

| 属性 | 存在性 | 类型 |
|---|---|---|
| `form` | 必需 | `"instructions"` |
| `kind` | 必需 | `"pto-artifact-analysis"` |
| `receipt` | 必需 | [`packages/host/pto-artifact-inspection/src/types.ts#PtoArtifactAnalysisReceipt`](#persistence-type-packageshostpto-artifact-inspectionsrctypestsptoartifactanalysisreceipt) |
...
|---|---|---|
| `actionId` | 必需 | `string` |
| `artifactRefs` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.artifactRefs`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptartifactrefs) |
| `recordId` | 必需 | `string` |
| `recordRevision` | 必需 | `string` |
| `requestId` | 必需 | `string` |
| `sessionId` | 必需 | `string` |
| `skill` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.skill`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptskill) |
| `tool` | 必需 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.tool`](#persistence-type-eventagentinboxspliceddatainserted0source14receipttool) |
...
| `provider` | 必需 | `string` |
| `reasoningEffort` | 可选 | `string` |
| `stop` | 可选 | [`event:agent/inbox/spliced.data.inserted[0].source[14].receipt.artifactRefs`](#persistence-type-eventagentinboxspliceddatainserted0source14receiptartifactrefs) |
...
归档命令捕获全部声明的持久化根及可达类型。此引用不证明迁移安全，也不代表 V3 历史中每次 payload 修订。离线格式检查器验证完整性和生成区域。
