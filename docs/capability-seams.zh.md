...
| `ctx.ptoExperiments` | `core` | [`pto-experiments`](../packages/pto/pto-experiments) | - | - | - | 负责 Workspace 本地提案与查询，以及可信 Host 的执行准入：干净的 Git/PyPTO 身份、绑定修订的用户回执、独占候选预留、命令执行和可识别的终态运行。模型仅保留规划与查询工具。 |
| `ctx.ptoArtifactInspection` | `core` | [`host-pto-artifact-inspection`](../packages/host/pto-artifact-inspection) | - | [`api-remotes`](../packages/api/remotes), [`client-ui-workspace`](../packages/client/ui-workspace) | - | 负责显式目录注册、修订检查、可撤销的静态查看路由及限定的官方依赖分析准入。 |
| `ctx.ptoExperimentDashboard` | `seam` | [`host-pto-experiment-dashboard`](../packages/host/pto-experiment-dashboard) | - | [`api-remotes`](../packages/api/remotes) | - | 从现有 Session 解析 Workspace 权限，提供有界只读实验投影，不接受调用方路径或存储身份键。 |
