...
const req = (query = '') => ({ query, position: 'leading' as const, signal: new AbortController().signal })
...
describe('draft composer catalog source', () => {
  ...
  it('projects Commands first and Skills second with origin, search, contained errors, and canonical pick text', async () => {
    ...
    expect(b.source.onPick({ candidate: all[0]!, session: target(), position: 'leading', via: 'menu', span: { start: 0, end: 0, draftRev: 1 } }))
      .toEqual({ text: '/analyze ' })
    expect(b.source.onPick({ candidate: all[2]!, session: target(), position: 'leading', via: 'menu', span: { start: 0, end: 0, draftRev: 1 } }))
  ...
  it('keeps a Command and Skill with the same name in their own sections and disambiguates pick text', async () => {
    ...
    const pick = (candidate: InputTriggerCandidate) => b.source.onPick({
      candidate, session: target(), position: 'leading', via: 'menu', span: { start: 0, end: 0, draftRev: 1 },
    })
