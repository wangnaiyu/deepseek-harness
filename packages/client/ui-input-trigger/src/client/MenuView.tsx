...
export function MenuView({
  menu, headers, onPick, onCrumb, onHover, onDismiss, onRetry = () => {}, t,
}: MenuViewProps) {
                      ...
                      <Fragment key={optionId(group.source, index)}>
                        ...
                        >
                          <span className={css.itemIcon} aria-hidden>
                            {item.icon === undefined ? null : typeof item.icon === 'string'
                              ? <ReferenceIconRegular kind={item.icon} size={16} /> : <item.icon size={16} />}
                          </span>
