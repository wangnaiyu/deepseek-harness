...
Workspace selection retargets the browser draft through `uiWorkspace.selectDraftWorkspace` without creating a Session. First-send materialization retains the new Session and awaits its binding before applying ordered preparation and transferring the captured payload.
...
A blank Session retains the header's leading and corner controls, including the right-sidebar opener, while hiding its title, actions, utilities, and View tabs. Selecting a Workspace only stages a draft; these controls become available when first send materializes the Session. Without a selected Session, the strict header is absent. Sidebar entries retain their own data and execution prerequisites.
