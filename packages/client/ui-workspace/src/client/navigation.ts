...
class UiWorkspaceService extends Service implements UiWorkspace {
  ...
  constructor(
    ...
    private readonly connectionGeneration?: ConnectionGenerationState,
  ) {
    ...
    ctx.effect(() => {
      ...
      const disposeGeneration = this.connectionGeneration?.subscribe(() => {
        const draft = this.list.getSnapshot().sessionDraft
        const cwd = this.connectionGeneration?.getSnapshot()?.host.cwd
        if (draft !== undefined && draft.workspaceId === undefined
          && cwd !== undefined && draft.cwd !== cwd) {
  ...
  private beginSessionDraft(workspaceId: WorkspaceId | undefined): void {
    ...
    const hostCwd = this.connectionGeneration?.getSnapshot()?.host.cwd
    this.setSessionDraft({
