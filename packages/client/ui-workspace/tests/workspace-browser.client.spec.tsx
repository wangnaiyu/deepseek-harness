...
describe('WorkspaceBrowser', () => {
  ...
  it('anchors collapsed drags before hidden rows so the source stays visible', async () => {
    ...
    const insertSessionBefore = vi.fn(async () => {})
    const b = mount({
      useSessions: hook(sessionState(ordinary, { current: ordinary[0]?.id })),
      useWorkspaces: hook(workspaceState([workspace('alpha', ordinary.map(item => item.id))])),
      insertSessionBefore,
    })
    await waitFor(() => {
      expect(b.store.getSnapshot().sessionOrderByAccount.alpha)
        .toEqual(['session-1', 'session-2', 'session-3', 'session-4', 'session-5', 'session-6'])
    })
    ...
    expect(b.store.getSnapshot().sessionOrderByAccount.alpha)
      .toEqual(['session-1', 'session-2', 'session-3', 'session-5', 'session-4', 'session-6'])
  ...
  it('promotes a New Session only when its first prompt materializes the row', async () => {
    const items = [
      summary('old', 100),
    ...
    rerender(b, {
      useSessions: hook(sessionState([
        summary('old', 100), summary('blank', 250), summary('mid', 200),
      ], { current: sid('blank') })),
    })
    await waitFor(() => {
  ...
  it('does not repeat materialization promotion after a manual drag', async () => {
    const insertSessionBefore = vi.fn(async () => {})
    const b = mount({
      useSessions: hook(sessionState([
        summary('old', 100),
