...
describe('WorkspaceBrowser', () => {
  ...
  it.each(['workspace', 'flat'] as const)('hides blank Sessions and keeps time ties stable in %s recency', (groupBy) => {
    localStorage.clear()
    const preferences = createWorkspaceViewStore().create()
  ...
  it('always renders the import bucket and hangs the open entry on it', () => {
    ...
    // bucket — which carries both the open entry and its expanded empty state.
    const rows = screen.getAllByRole('treeitem')
    expect(rows.map(row => row.textContent)).toEqual(['alpha', '未分组'])
    expect(screen.queryByText('暂无运行记录')).toBeNull()

    fireEvent.click(rows[1] as HTMLElement)
    const empty = screen.getByText('暂无运行记录')
    expect(rows[1]?.parentElement?.contains(empty)).toBe(true)
    fireEvent.click(rows[1] as HTMLElement)
    expect(screen.queryByText('暂无运行记录')).toBeNull()
  ...
  it('workspace hover card shows a POSIX home descendant as ~', () => {
    ...
    try {
      ...
      fireEvent.pointerEnter(projectRow.parentElement as HTMLElement)
  ...
  it('reconciles a late blank and its first prompt while the sidebar is collapsed', async () => {
    ...
    rerender(b, { wide: true })
    expect(screen.getAllByRole('treeitem').filter(row => row.getAttribute('aria-expanded') === null).map(row => row.textContent)).toEqual([
  ...
  it('keeps the blank New Session out of navigation until it materializes', () => {
  ...
  it('keeps blank Sessions hidden while collapsed drags retain the source row', async () => {
    const ordinary = Array.from({ length: 6 }, (_, index) => summary(`session-${index + 1}`, 6 - index))
    const blank = summary('blank', 7, { blank: true })
    const b = mount({
      useSessions: hook(sessionState([blank, ...ordinary], { main: blank.id })),
    ...
    expect(screen.queryByText('新会话')).toBeNull()
    const first = screen.getByText('session-1').closest('[role="treeitem"]') as HTMLElement
  ...
  it('auto-expands the Ungrouped bucket and its ＋ starts an unassigned session', () => {
    const startSession = vi.fn()
  ...
  it('keeps all blank sessions out of grouped and flat navigation', () => {
  ...
  it.each(['workspace', 'flat', 'ungrouped'] as const)('hides new blanks across %s mode switches and enables drag after the first prompt', (mode) => {
    localStorage.clear()
    const preferences = createWorkspaceViewStore().create()
    preferences.actions.setGroupBy(mode === 'flat' ? 'flat' : 'workspace')
    const account = mode === 'flat' ? FLAT_SESSION_ORDER_KEY : mode === 'ungrouped' ? UNGROUPED_KEY : 'alpha'
  ...
  it('nests the grouped no-sessions state under Ungrouped, keeps the flat empty state, and resolves an empty search', async () => {
    vi.useFakeTimers()
    try {
      const b = mount()
      expect(screen.queryByText('暂无会话')).toBeNull()
      const ungrouped = screen.getByText('未分组').closest('[role="treeitem"]') as HTMLElement
      fireEvent.click(ungrouped)
      const empty = screen.getByText('暂无会话')
      expect(ungrouped.parentElement?.contains(empty)).toBe(true)
      fireEvent.click(ungrouped)
      expect(screen.queryByText('暂无会话')).toBeNull()
      b.store.actions.setGroupBy('flat')
      rerender(b, {})
