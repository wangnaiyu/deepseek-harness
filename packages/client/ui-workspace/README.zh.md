...
区头之上有**会话**和**运行记录**两个 Tab；轨道两者都不承载，因此折叠侧边栏会让该区域回到会话，并连同运行记录的查询一起清除。运行记录在结构上是同一棵树——按 Host 顺序为每个已注册 Workspace 各一节，其后是 Ungrouped——但动词刻意更少：区头只有搜索（没有新会话、没有视图选项、也没有分组方式，因为脱离工程的 run 列表没有可分组的维度），行既无排序也无拖拽，行本身不可点击。运行记录是用来回溯与挑选的对象，不是任务起点。这里的 Ungrouped 是**导入桶**，不是会话树的溢出桶：它在为空时也照常渲染，因为打开 run 目录的尾部 hover 显现 `＋` 只挂在该行上——工程分节不提供，其下的 run 由扫描得到，不能手工添加。打开一个 run 目录只登记一条浏览器本地记录，不创建 Workspace（注册延迟到在该记录上开出第一个会话时），其分组在这一刻一次性判定：命中包含该路径的最内层已注册 Workspace，否则进入导入桶。该归属是黏性的——之后再注册父工程不会移动这条记录，父工程自身的扫描会把同一目录作为它自己的行呈现，因为从两张行开出的会话 cwd 不同。移除一条记录只是摘掉登记，既不动目录也不动其中的文件，因此确认框会写明这一边界。区头搜索框按名称和路径过滤记录；过滤视图中没有命中的工程分节会隐去，而导入桶保留，使打开入口始终可达。

<a id="use-this-package"></a>
## 使用本包

用侧边栏浏览 Workspace 及其 Session、重排它们并新建会话；在 Session Intent 主视觉区用选择器为新会话选择 Workspace。打开的 Workspace 默认显示五条非空白 Session，并在首条提示词落地前隐藏所有空白 Session。**展开其余**会显示隐藏条目；关闭再打开 Workspace 会恢复该折叠投影。

### 重排序与视图选项

**最近更新**在分组和单列表视图中都按普通 Session 的最近一次用户提示词或 steering（中途引导）时间降序排列。**手动排序**会冻结当前显示顺序，并在活动变化时保持已有位置；新发现的普通 Session 追加到末尾，多条同时到达时按最近更新时间降序排列。返回最近更新会丢弃所有手动位置；再次进入手动排序时，则冻结当时的最近更新顺序。浏览器默认采用最近更新，并在重新加载后记住所选模式。拖拽普通 Session 会在浏览器本地应用移动并选中手动排序。空白 Session 不渲染行。首次提示词之后，Session 按所选排序模式成为普通可拖拽行。折叠分组的拖拽边界按渲染行确定，并把来源行放在中间隐藏行之前，因此拖拽不会隐藏来源行。真实 Workspace、Ungrouped 与单列表的 Session 显示顺序都保留在浏览器本地；Workspace 分组的拖拽顺序仍由 Host 持久化。
