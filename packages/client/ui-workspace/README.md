...
The region carries two Tabs, **Sessions** and **Run Records**, above the section header; the rail carries neither, so collapsing the sidebar returns the region to Sessions and drops any run-record query with it. Run Records is structurally the same tree — one section per registered Workspace in Host order, then Ungrouped — and deliberately poorer in verbs: its header offers search alone (no new session, no view options, no grouping, because a run list detached from a project has nothing to group by), rows carry no order and no drag, and a row itself is inert. A run record is something to look back through and pick from, not a task start. Ungrouped here is an **import bucket**, not the session tree's overflow bucket: it renders even while empty, because the trailing hover-revealed `＋` that opens a run directory hangs on that row and nowhere else — a project section offers none, since the runs under it are scanned rather than added by hand. Opening a run directory registers a browser-local record and creates no Workspace (the registration is deferred to the first session started on the record), and its group is resolved once, at that moment: the innermost registered Workspace containing the path, else the bucket. That attribution is sticky — registering a parent project later leaves the record where it was opened, and the parent's own scan surfaces the same directory as its own row, because sessions started from the two rows differ in cwd. Removing a record deregisters it and touches neither the directory nor its contents, so the confirmation states that boundary. The header search field filters records over label and path; a project section without a match drops out of the filtered view while the bucket stays, keeping the open entry reachable.

<a id="use-this-package"></a>
## Use this package

Use the sidebar to browse Workspaces and their Sessions, reorder them, and start new ones; use the picker in the Session Intent hero to choose a Workspace for a new session. An open Workspace shows five non-blank Sessions by default and hides all blank Sessions until their first prompt. **Show more** reveals the hidden remainder; closing and reopening the Workspace restores this folded projection.

### Reordering and view options

**Last updated** orders ordinary Sessions by their latest user prompt or steer time, newest first, in both grouped and flat views. **Manual** freezes the current displayed order and holds positions when activity changes; newly discovered ordinary Sessions append to the end, newest first when several arrive together. Returning to Last updated discards every manual position, and entering Manual again freezes the then-current recency order. The browser defaults to Last updated and remembers the selected mode across reloads. Dragging an ordinary Session applies the move locally and selects Manual. Blank Sessions do not render rows. After its first prompt, a Session becomes an ordinary draggable row using the selected ordering mode. In a collapsed group, drag boundaries follow rendered rows and place the source before intervening hidden rows, so a drag cannot hide its source. Session display orders for real Workspaces, Ungrouped, and the flat list are browser-local; Workspace group drag order remains Host-durable.

### Workspace hierarchy

Choose **Add workspace** and select a directory to register it and stage a browser draft. **View options → Group by** defaults to **WorkSpace**, which lists Workspaces as sibling sections. Select **Workspace Tree** to nest each Workspace under its nearest registered ancestor, including Workspaces added later. Each Workspace keeps its own Sessions and row actions. Child Workspaces appear before the parent's own Sessions. Ancestors start expanded unless a saved collapsed state exists. A saved collapse also hides the current Session; ancestor folder icons stay highlighted when a descendant Workspace contains it. Row fills and hit targets span the same width at every level; only the contents indent. Workspace dragging reorders siblings; dropping on a descendant targets the nearest compatible ancestor, so an expanded parent can be moved past without collapsing it. Search-result navigation expands every ancestor. Grouping and expansion are saved in the current browser; switching modes preserves each Workspace's expansion preference, and the single-list view stays flat.
...
- **Run records list only what the user opened by hand** — the on-demand recognizer scan that would populate the project sections is the discovery layer's work and is not built yet, so those sections render empty until a record is opened into them.
- **Run-record attribution compares paths exactly** — a case-insensitive filesystem is not modeled, so a path that differs only in case from its registered Workspace lands in the import bucket instead of that project's section.
