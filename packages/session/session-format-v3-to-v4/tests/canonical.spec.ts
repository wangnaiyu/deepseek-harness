...
function source() {
  const user = { id: 'context',
    ...
    content: [{ type: 'text',
      text: 'context' }] }
  const dispatch = { rootCallId: 'root',
    parentCallId: 'root',
    subCallId: 'child',
    name: 'test',
...
describe('canonical V4 integration', () => {
  it('composes seeded systems, source ownership, PTC, canonical replacements and native round-trip', () => {
    ...
    expect(users[1]).toMatchObject({ surfaceOp: { op: 'replace',
      startSeq: 5,
      endSeq: 5 },
    sourceEventSeqs: [5],
    data: { source: { kind: 'ptc-mode' } } })
    expect(target.events.find(event => event.type === 'tool/ptc-dispatch')?.data).toMatchObject({ arguments: { kind: 'plugin',
      plugin: 'tools-ptc' } })
    const before = JSON.stringify(target)
    expect(restoreReleasedV4Artifact({ ...target,
      header: { ...target.header,
        version: 4 } },
    new Set(target.events.map(event => event.type))).events).toBe(target.events)
    expect(reopen(target)).toEqual(target)
    expect(JSON.stringify(target)).toBe(before)
  })

  it.each([{ sourceEventSeqs: [] },
    { surfaceOp: { op: 'replace',
      start: 0,
      end: 0 } }])('rejects noncanonical native metadata %j',
    (override) => {
      const target = migrated()
  ...
  it('keeps earlier-generation delivery markers ignored and validates native current ownership', () => {
    const marker = (version: number): SessionFormatEvent => ({ type: 'session-log-deepseek/delivery-accepted',
      seq: 0,
      time: 1,
    ...
    expect(() => restoreReleasedV4Artifact({ ...artifact,
      ...
      { ...marker(4),
        seq: 1 }] },
    new Set(['feedback/record',
      marker(4).type]))).toThrow(/wrong Session/)
