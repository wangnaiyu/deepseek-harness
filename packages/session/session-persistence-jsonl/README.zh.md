...
PTO fork 写入 V5。`legacyPtoV4` 默认 `false`，用于上游 V4 根目录；仅对升级前 PTO fork 产生的根目录设为 `true`。一个根目录不能混用两条 V4 lineage。只读迁移不发布文件；可写迁移经过校验，在不改动历史代的前提下发布 V5。见 [lineage 选择](../session-format-v4-to-v5/README.zh.md#lineage-selection)。
