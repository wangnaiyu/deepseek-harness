...
The PTO fork writes V5. `legacyPtoV4` defaults to `false` for upstream V4 roots; set it to `true` only for a root produced by the pre-upgrade PTO fork. Do not mix the two V4 lineages in one root. Read-only migration never publishes files; writable migration verifies and publishes V5 beside unchanged historical generations. See [lineage selection](../session-format-v4-to-v5/README.md#lineage-selection).
