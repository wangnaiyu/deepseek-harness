...
describe('combined structural, canonical-envelope and PTC catalog migration', () => {
  it('inserts and clears system prompts while remapping replacement and nested PTC history without rewriting identities', () => {
    ...
    const released = { ...target, header: { ...target.header, version: 3 } }
    expect(restoreReleasedV3Artifact(released, new Set())).toBe(released)
    ...
    expect(() => restoreReleasedV3Artifact({ ...released, events: mismatched }, new Set())).toThrow(/parentCallId/)
  })
  ...
  it.each([undefined, 0, 1, 2, 4])('preserves historical delivery generation %s while remapping its envelope and inherited cut', (version) => {
    ...
    assertComposite(target.events)
    const reopened = sessionFormatCatalog.createRestore(releasedV3SessionFormatCodec.encodeHeader({ ...target.header, version: 3 }, target.inheritedEventCount), { recovery: 'strict', validation: 'current' })
