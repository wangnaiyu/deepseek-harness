...
规范的用户 Skill 语法是 `/skill <name>`。`session.prompt` 在把直接用户文本持久化前，会按精确的实时 Session registry 与 `userInvocable` 策略复核每个规范名称。发现失败返回 `skill-catalog-unavailable`；名称缺失或刚被禁止用户调用时返回 `skill-unavailable`。两种失败都不接受 prompt，使客户端可以保留草稿、显示错误并重试。后续 pre-step 仍负责加载和注入正文。
