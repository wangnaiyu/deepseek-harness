...
Read-only Host projection for a new-session composer draft. `ComposerCatalogGateway` registers the `composerCatalog` service and publishes `composerCatalog/listDraft`; the request carries only an optional `workspaceId` and Agent preset id, never a client path. The Host resolves a known Workspace to its canonical path and display title, addresses the preset standing composition without creating an Agent, then returns effective Commands and user-invocable Skills in separate arrays.
